...
class Peer(Process):
        ...
        while True:
            ...
            try:
                if len(self.trading_queue)>0:
                    ...
                    print(f"Trader{self.current_trader_id.index(self.id)} cache: {self.cache}")
                    ...
                    # Try to fetch the item from cache if possible otherwise serve from db

                    data,cache_status = self.fetch_item_from_cache(item)
                    if not cache_status:
                    ...
                    if cache_status:
                        print(f"{self.get_timestamp()} {item} is served from cache by Trader{self.current_trader_id.index(self.id)}")
                    else:
                        print(f"{self.get_timestamp()} {item} is served from database by Trader{self.current_trader_id.index(self.id)}")
    ...
    def send_sale_message(self, item, commission, count, buyer_id, zero_flag):
        ...
        if count <=0:
            ...
            self.has_deposited_lock.release()
            self.begin_trading()
    def fetch_item_from_cache(self,item):
    ...
    def register_product(self, seller_info, seller_id):
        ...
        try:
            ...
            if seller_data == None or seller_data["count"] == 0:
                return False
            # Add the newly added product to cache too

            self.cache.append(seller_info)
            return True
        except Exception as e:
